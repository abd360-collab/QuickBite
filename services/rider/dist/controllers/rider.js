...
export const addRiderProfile = TryCatch(async (req, res) => {
    ...
    if (existingProfile) {
        return res.status(400).json({
            message: "Rider's profile already exist",
        });
    }
...
export const toggleRiderAvailability = TryCatch(async (req, res) => {
    ...
    if (user.role !== "rider") {
        return res.status(403).json({
            message: "Only riders can toggle status",
        });
    }
...
export const acceptOrder = TryCatch(async (req, res) => {
    ...
    try {
        ...
        if (data.success) {
            await Rider.findOneAndUpdate({ userId: riderUserId, isAvailable: true }, { isAvailable: false }, { new: true });
        }
        res.json({
        ...
        res.status(400).json({
            message: "Order already taken or failed to assign",
        });
    }
});
