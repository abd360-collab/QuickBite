    ...
    <div className="mx-auto max-w-5xl px-4 py-5 space-y-6">
      <div className="rounded-xl bg-white p-4 shadow-sm">
        ...
        <p className="text-sm text-gray-500">
          {restaurant.autoLocation.formattedAddress}</p>
      </div>
        ...
        {cart.map((cartItem: ICart) => {
          ...
          return <div key={item._id} className="flex items-center gap-4 rounded-xl bg-white p-4 shadow-sm">
              ...
              <div className="flex items-center gap-3">
                <button className="rounded-full border p-2 hover:bg-gray-100 disabled:opacity-50" disabled={isLoading} onClick={() => decreaseQty(item._id)}>{isLoading ? (<VscLoading size={16} className="animate-spin" />) : 
               ( <BiMinus size={16} /> )}</button>

               <span className="font-medium">{cartItem.quantity}</span>

                 <button className="rounded-full border p-2 hover:bg-gray-100 disabled:opacity-50" disabled={isLoading} onClick={() => increaseQty(item._id)}>{isLoading ? (<VscLoading size={16} className="animate-spin" />) : 
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
        ...
        <button className={`mt-3 w-full rounded-lg bg-[#E23744] 
        ...
        <button className="mt-3 w-full rounded-lg bg-[#7b7979] py-3 text-sm font-semibold text-white hover:bg-gray-500 flex justify-center items-center gap-3" 
        onClick={clearCart} disabled={clearingCart}>
